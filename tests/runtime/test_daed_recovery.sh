...
stop)
	set -- "$ATHENA_ROOT"/root/athena-backups/daed-recovery-*
	[ -d "$1" ] && (cd "$1" && sha256sum -c checksums.sha256 >/dev/null) || exit 1
...
reset_fixture() {
	...
	unset DAED_INIT_RUNNING DAED_STOP_STUCK DAED_API_STUCK DAED_STOP_RESULT DAED_RESULT DAED_USERNAME DAED_SECOND_USERNAME DAED_RESET_SLEEP DAED_EXTRA_BLANK DAED_SHA_FAIL
...
! grep -q '^stop$\|resetpass' "$ATHENA_RECOVERY_TEST_LOG" || fail checksum_failure_side_effect
[ ! -e "$ROOT/var/lock/athena-daed-recovery.lock" ] || fail checksum_failure_unlock

reset_fixture; export DAED_RESULT=fail; run_recovery 'RESET DAED PASSWORD'; assert_failure reset_failure_status
[ "$(cat "$RUNNING_FILE")" = 1 ] || fail reset_failure_restore
...
reset_fixture; run_recovery 'RESET DAED PASSWORD'; [ "$RECOVERY_STATUS" -eq 0 ] || fail success_status; assert_one_json
...
reset_fixture; cli_output="$(printf 'RESET DAED PASSWORD\n' | "$CLI" 2>"$ROOT/cli.err")" || fail cli_interactive
grep -q 'WARNING:' "$ROOT/cli.err" || fail cli_warning
printf '%s' "$cli_output" | "${PYTHON:?set PYTHON}" -c 'import json,sys; json.load(sys.stdin)' || fail cli_json
