...
## v19.0.0-rc2 DAED Web 源码预检修复

- 修复第 24 次构建在 `Assemble pinned DAED source` 阶段的误报：编译后的 GraphQL 客户端依赖可以正常包含 `ClientError`、请求结构字段与控制台方法，不再仅凭这些库符号判定登录凭据泄漏。
- 登录页 `Setup.tsx` 仍在构建前按固定提交与精确摘要执行严格检查，继续禁止直接显示错误对象、请求变量、请求体、`ClientError` 或序列化异常。
- 编译产物与 gzip 嵌入资源继续拒绝凭据哨兵、直接错误 toast 和错误对象序列化；失败日志现在只报告安全规则名称与资源路径，不回显敏感内容。
- 新增静态 UI 与嵌入 Web 的回归用例，同时证明正常 GraphQL 运行时代码可通过、真实凭据哨兵仍会被拒绝。
...
- `athena-setup` 仅执行安全预检、备份、运行时策略和严格健康验证；订阅、节点、DNS 与路由均在原生 DAED UI 中管理。
