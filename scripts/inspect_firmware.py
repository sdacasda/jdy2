...
def main():
 ...
 report={"initramfs":[str(x) for x in init],"sysupgrade":[str(x) for x in sysup],"kernel_bytes":size,"kernel_limit":a.kernel_limit,"kernel_margin":a.kernel_limit-size,"missing_packages":sorted(missing),"forbidden_packages":sorted(FORBIDDEN&packages)}
