...
## 2026-08-11：IPv6 与 DAED 连接状态更正

- IPv6 首页状态现在合并检查 `wan` 和 `wan6`。移动宽带常由 `wan6` 持有公网 IPv6 与 PD 前缀，因此旧实现只读取 `wan` 会产生假阴性。
- DAED 面板的显示门槛改为“核心进程是否运行”。核心停止时只显示“后端未连接”；核心运行时显示完整原生 UI，即使辅助 API 探针尚未确认。
- API 探针保留 GraphQL POST，并增加真正的 `wget -> nc` 降级路径，以兼容裁剪选项不同的 BusyBox。
- `:2023` 仍只允许路由器回环访问；用户入口保持 LuCI 内嵌的 `/athena-daed/`。
